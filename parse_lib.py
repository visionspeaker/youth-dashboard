...
MAIN_BAN = {"중1","중2","중2남","중2여","중3","고1","고2","고3"}  # 학생 반(새친구·영어반 제외)
# 영어반: 8/16 등반 후 8/23 부터 앨런·반잭키·반레이첼이 학년 반 명단에 편입되어 이중계산 발생 → 집계 제외.
# (영어반 행은 8/16·8/23 뿐이고 결석명단이 비어 있어 과거 결석 이력에 영향 없음. weeks.json 과거 주차는 고정.)
NF_BAN = "새친구"
# 반 -> 학년 매핑 (중2남/중2여 -> 중2 로 합산)
BAN2GRADE = {"중1":"중1","중2":"중2","중2남":"중2","중2여":"중2","중3":"중3",
             "고1":"고1","고2":"고2","고3":"고3"}
NAME_FIX = {"반소영":"반잭키", "반재키":"반잭키"}  # 시트 오타 보정(compute.py norm 과 동일)

# 헌금 항목 정규화 (2026-09-21 반영)
# 원천 탭은 생일감사헌금을 '기타'로 입력하지만 사역보고서는 '감사헌금(생일)'로 분류한다.
# '기타' 중 명단에 '생일'이 들어간 행만 감사헌금으로 승격한다.
# 지목헌금(예: "지목헌금 박션")·찬조(예: "찬조 강순애 권사")는 '기타'로 그대로 둔다.
# ⚠ 리라벨 금지 규칙의 예외. 같은 날 기존 감사헌금 행과 금액까지 같으면
#    (날짜,항목,금액) 병합키가 겹쳐 행이 소실되므로 ingest 에서 반드시 검사한다.
def norm_item(item, names=""):
    it = (item or "").strip()
    if it == "기타" and "생일" in (names or ""):
        return "감사헌금"
    return it
